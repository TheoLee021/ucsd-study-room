...
끝! Claude에게 물어보세요: *"내일 오후 2시부터 4시까지 스터디룸 예약해줘"*

### 다른 MCP 클라이언트

다른 MCP 호환 AI 어시스턴트(Codex CLI, Gemini CLI, Cursor 등)를 사용 중이라면, 아래 프롬프트를 복사해서 붙여넣기만 하세요:
...
또는 수동: `uv tool install ucsd-study-room@latest`
...
| `study-room update`        | PyPI에서 최신 버전으로 업데이트                              |
